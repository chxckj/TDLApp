...
import { getLocale } from '../etc/Utility'
...
export const initDatabase = (callback) => {
	db.transaction(
		(tx) => {
			...
			tx.executeSql(
				'create table if not exists analysis (id integer primary key not null, name text, avatar text, endedTask integer);',
			)
			tx.executeSql(
			...
			tx.executeSql(
				"INSERT OR IGNORE INTO profile (id, name, avatar, endedTask) values (0, 'GUEST', '', 0);",
			)
			tx.executeSql(
